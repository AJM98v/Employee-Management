<div class="relative p-10 bg-gray-300" x-data="{
            ...
            <tr class="transition-all duration-75 ease-in bg-white cursor-pointer hover:bg-gray-200">
                ...
                <td class="flex">
                    ...
                    <button wire:click="delete({{$employee->id}})"
                            class="w-24 py-2 ml-2 text-sm text-white transition-all duration-200 ease-in rounded shadow cursor-pointer shadow-gray-500 bg-rose-700 hover:bg-red-600">
                        Delete
                    </button>
                </td>
            </tr>
    ...
    <div x-show="modal" x-transition.opacity.delay..200ms
         class="translate-x-[-50%] translate-y-[-50%] fixed top-[50%] left-[50%] bg-gray-300  p-4 rounded-xl shadow-gray-500 shadow drop-shadow">
        @if($editMode)
            <form class="grid grid-cols-2 gap-3" wire:submit.prevent="update()">
                @else
                    <form class="grid grid-cols-2 gap-3" wire:submit.prevent="store()">
                        ...
                        <textarea type="text" wire:model="address" id="address"></textarea>
                        <label for="country">Country</label>
                        <select wire:model="country_id" id="country" >
                            <option selected value="{{null}}">Please Choose</option>
                            @foreach($countries as $country)
                                <option value="{{$country->id}}"  >{{$country->name}}</option>
                            @endforeach
                        </select>
                        <label for="city">City</label>
                        <select wire:model="city_id" id="city">
                            <option selected value="{{null}}">Please Choose</option>
                        ...
                        <input type="tel" wire:model="zip_code" id="zip_code">
                        <label for="birthdate">Birth Day</label>
                        <input type="date" wire:model="birthdate" id="birthdate">
                        <label for="hired_date">Hire Day</label>
                        <input type="date" wire:model="hired_date" id="hired_date">
                        <button type="submit" @click="message = true"
                                class="w-40 py-2 text-white rounded shadow bg-sky-700 shadow-gray-500">@if($editMode)
                                Update @else Save @endif Employee
                        </button>

                    </form>
                    <button @click="modal=false; $wire.defualt()"
                            class="w-20 py-1 mt-2 ml-auto text-white bg-gray-800 rounded-xl">Close
                    </button>
    ...
    @if(\Illuminate\Support\Facades\Session::has('message'))
        <div class="absolute p-2 text-sm font-bold text-white bg-green-800 bottom-2 right-2 rounded-xl"
             x-init="setTimeout(()=>{message = false},3000)" x-show="message" x-transition.duration.300ms>
